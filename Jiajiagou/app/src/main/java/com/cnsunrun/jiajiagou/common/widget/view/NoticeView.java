...
public class NoticeView extends ViewFlipper implements View.OnClickListener
{
    ...
    public void addNotice(List<NoticeBean> notices)
    {
        ...
        {
            // 根据公告内容构建一个Tetview
            String notice = notices.get(i).title;
            TextView textView = new TextView(mContext);
            textView.setSingleLine();
            textView.setText(notice);
            textView.setTextSize(15f);
            textView.setEllipsize(TextUtils.TruncateAt.END);
            textView.setTextColor(0xff3ba4fa);
    ...
    public void onClick(View v)
    {
        ...
        String notice_id = mNotices.get(position).notice_id;

        if (mOnNoticeClickListener != null)
        {
            mOnNoticeClickListener.onNoticeClick(position, notice_id);
        }
    }
    ...
    {
        void onNoticeClick(int position, String notice_id);
    }
