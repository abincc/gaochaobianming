...
public class PropertyDetailActivity extends BaseHeaderActivity
{
    ...
    private void getData()
    {
        ...
        HttpUtils.get(NetConstant.CLEAN_PROPERTY, hashMap, new BaseCallBack(mContext)
        {
            @Override
            public void onResponse(String response, int id)
            {
                LogUtils.d(response);
                HelpDetailRespV2 resp = new Gson().fromJson(response, HelpDetailRespV2.class);
                if (handleResponseCode(resp))
                {
                    HelpDetailRespV2.InfoBean info = resp.getInfo();
                    ...
                    List<HelpDetailRespV2.InfoBean.ImagesBean> images = info.getImages();
                    setBanner(images);
                    if (resp.getInfo().getIs_reply().equals( "1")) {//已回复
                        mTvReplyContent.setVisibility(View.VISIBLE);
                        mTvReplyContent.setText("回复时间："+resp.getInfo().getReply());
                    } else {
                        mTvReplyContent.setVisibility(View.GONE);
                    }
                    List<HelpDetailRespV2.InfoBean.ReplyImagesBean> replyImages = resp.getInfo().getReply_images();
                    if (replyImages != null&&replyImages.size()>0){

                        mNgvReply.setVisibility(View.VISIBLE);
                        List<ImageInfo> urlList = new ArrayList();
                        for (HelpDetailRespV2.InfoBean.ReplyImagesBean replyImage : replyImages) {
    ...
    private void setBanner(List<HelpDetailRespV2.InfoBean.ImagesBean> images)
    {
        ArrayList<String> pics = new ArrayList<>();

        for (HelpDetailRespV2.InfoBean.ImagesBean imageBean : images)
        {
    ...
    public void onViewClicked()
    {
        ...
        hashMap.put("token", JjgConstant.getToken(mContext));
        hashMap.put("trifle_id", mTrifle_id);
        HttpUtils.post(NetConstant.TRIFLE_DETAIL_PROPERTY, hashMap, new BaseCallBack(mContext)
        {
            @Override
            public void onResponse(String response, int id)
            {
                ...
                if (handleResponseCode(baseResp))
                {
                    ...
                    bundle.putString(ConstantUtils.FROM_CLEAN_OR_LITTLE, ConstantUtils.FROM_LITTLE);
                    bundle.putString("id", mTrifle_id);
