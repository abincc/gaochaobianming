...
public class BaseOrderFragment extends BaseFragment implements BaseQuickAdapter.RequestLoadMoreListener
{
...
//        mAdapter.setNewData(mList);
    ...
    private void getData(final int page)
    {
        ...
        if (type == 1)
        {
            Url = NetConstant.ALLORDER;
        } else if (type == 2)
        {
            Url = NetConstant.UNPAID;
        } else if (type == 3)
        {
            Url = NetConstant.RECEIVE;
        } else if (type == 4)
        {
            Url = NetConstant.EVALUATE;
        }
        HashMap<String, String> hashMap = new HashMap<>();
        hashMap.put("token", JjgConstant.getToken(mContext));
        hashMap.put("p", String.valueOf(page));
        HttpUtils.get(Url, hashMap, new DialogCallBack((Refreshable) BaseOrderFragment.this)
        {
            @Override
            public void onResponse(String response, int id)
            {
