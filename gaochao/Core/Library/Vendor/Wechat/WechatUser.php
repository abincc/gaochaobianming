...
class WechatUser extends Common {
    ...
    /* 批量获取粉丝信息*/
    const USER_BATCHGET_URL = '/user/info/batchget?';
     ...
     * @return bool|array
     */
    public function batchget($openids)
    {
        if (!$this->access_token && !$this->getAccessToken()) {
            return false;
        }
        $result = Tools::httpPost(self::API_URL_PREFIX . self::USER_BATCHGET_URL . "access_token={$this->access_token}",Tools::json_encode($openids));
        if ($result) {
            $json = json_decode($result, true);
            if (isset($json['errcode'])) {
                $this->errCode = $json['errcode'];
