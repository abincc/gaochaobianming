...
exit;
    ...
    //APP支付使用
    public function create_sign($prepayid){

        $option = array();
        $option["appid"]        = $this->appid;
        $option["partnerid"]    = $this->mch_id;
        $option["prepayid"]     = $prepayid;
        $option["package"]      = "Sign=WXPay";
        $option["noncestr"]     = Tools::createNoncestr();
        $option["timestamp"]    = (string)time();
        $option["sign"]         = Tools::getPaySign($option, $this->partnerKey);

        write_debug(array_merge($option,array('prepayid'=>$prepayid)),'微信支付签名');
        return $option;
    }    
