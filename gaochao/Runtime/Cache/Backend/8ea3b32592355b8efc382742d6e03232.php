			...
			<tr>
				<th colspan="9" class="clearfix">
					<form action="<?=U('index');?>" class="form-inline pull-left" method="get">
						<input type="text" class="form-control input-md" style="width: 250px;" placeholder="用户姓名|联系电话" name="keywords" value="<?=$keywords?>">
						<?php
 if(!session('is_owner')){ ?>
						...
						<?php }?>
						<select name="trifle_service_id" class="form-control input-sm">
							<option value=''>请选择服务类型</option>
							<?php
 foreach ($service as $k => $v) { ?>
							<option value="<?=$v['id']?>" <?=(I('trifle_service_id') == $v['id'])?'selected="selected"':''?> ><?=$v['title']?></option>
							<?php
 } ?>
						</select>
 ...
 foreach ($status as $k => $v) { ?>
		...
		<tbody>
			<tr role="row">
				...
				<th width="8%" class="text-center">回复</th>
...
<div class="noneshowlist text-center" style="height:auto;line-height:normal;">
 ...
 foreach($list as $row){ ?>
			<tr>
				...
				<td><?=$row['mobile']?></td>
				<td><?=$row['title']?></td>
				<td><?=$service[$row['trifle_service_id']]['title']?></td>	
				<td class="text-center"><?=$row['add_time']?></td>
				<td class="text-center"><?=$status[$row['status']]['title']?></td>
				<td class="text-center"><?=($row['is_reply'])?'已回复':'未回复'?></td>
