	...
	<table class="table js_table table-bordered">
			...
			<tr>
				<th colspan="20" class="clearfix">
					<form action="<?=U('index');?>" class="form-inline pull-left" method="get">
						<input type="text" class="form-control input-md" style="width: 250px;" placeholder="名称 | 电话号码" name="keywords" value="<?=$keywords?>">
		...
		<tbody>
			<tr role="row">
				...
				<th width="12%"  class="text-center">小区</th>
				<th width="8%" class="text-center">ip</th>
				<th width="6%" class="text-center">port</th>
				<th width="6%"  class="text-center">版本号</th>
				<th width="6%" class="text-center">通道号</th>
				<th width="12%" class="text-center">通道标题</th>
...
<div class="noneshowlist text-center" style="height:auto;line-height:normal;">
 ...
 foreach($list as $row){ ?>
			<tr>
				...
				<td class="text-center"><?=($row['district_id'])?$district[$row['district_id']]['title']:'<span style="color:green;">全部小区</span>'?></td>
				...
				<td class="text-center">
					<img src="/gaochao/Public/Backend/img/status_<?=$row['is_hid']?>.png" alt=<?=$row['is_hid']==0?"未禁用":"已禁用"?> title=<?=$row['is_hid']==0?"未禁用":"已禁用"?>>
				</td>
				<td class="text-center">
				<?php
