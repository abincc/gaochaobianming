...
<div class="message_bg js_message_bg"></div>
	...
	<div class="box box-body">
		...
		<form action="" method="post" class="js_form">
			...
			<label class="form-group">
				...
				<div>
					<select name="role_id" class="form-control width_auto">
						<?=$info['role_select'];?>
					</select>
					<span class="Validform_checktip">选择用户组</span>
				</div>
			</label>
			<label class="form-group">
				<div class="control-label">
				...
				<div>
					<input type="text" class="form-control width_auto" 
						datatype="*" nullmsg="填写用户名"
						value="<?=$info['username']?>" name="username" placeholder="用户名"/>
					<span class="Validform_checktip">用户名为6-20位</span>
				</div>
			</label>
			<label class="form-group">
				<div class="control-label">
 ...
 if(!$info['id']){ ?>
			...
			<label class="form-group district_select <?=($info['role_id'] != 1 && $info['role_id'] != '')?'':'hide'?>" >
				<div class="control-label">
					<span class="required">*</span>
					小区
				</div>
				<div>
					<select name="district_ids[]" class="form-control input-md super_select2 width_auto" data-placeholder="请选择小区" multiple >
						<?=$district?>
					</select>
					<span class="Validform_checktip">选择小区</span>
				</div>
			</label>
			<label class="form-group company_select <?=($info['role_id'] == 2)?'':'hide'?>"">
				<div class="control-label">
					<span class="required">*</span>
					公司
				</div>
				<div>
					<select name="company_id" class="form-control width_auto">
						<?=$company_select?>
					</select>
					<span class="Validform_checktip">请选择公司</span>
				</div>
			</label>
			<label class="form-group">
				<div class="control-label">
	...
	<script type="text/javascript">
		...
		$("select[name='role_id']").change(function(){
		    ...
		    if(value =='1')
		    {
		       $(".district_select").addClass('hide');
		    }else
		    {
		       $(".district_select").removeClass('hide');
		    }
