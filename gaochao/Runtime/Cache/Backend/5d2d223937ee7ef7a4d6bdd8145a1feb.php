...
<div class="message_bg js_message_bg"></div>
 ...
 if(!session('is_owner')){ ?>
						...
						<?php }?>
						<select name="clean_service_id" class="form-control input-sm">
							<option value=''>请选择服务类型</option>
							<?php
 foreach ($service as $k => $v) { ?>
							<option value="<?=$v['id']?>" <?=(I('clean_service_id') == $v['id'])?'selected="selected"':''?> ><?=$v['title']?></option>
							<?php
 } ?>
						</select>
...
<div class="noneshowlist text-center" style="height:auto;line-height:normal;">
 ...
 foreach($list as $row){ ?>
			<tr>
				...
				<td><?=$row['mobile']?></td>
				<td><?=$row['title']?></td>
				<td><?=$service[$row['clean_service_id']]['title']?></td>	
