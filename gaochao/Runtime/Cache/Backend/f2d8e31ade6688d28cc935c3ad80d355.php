 ...
 foreach ($status as $k => $v) { ?>
		...
		<tbody>
			<tr role="row">
				...
				<th width="12%" >下单用户</th>
				<th width="12%" class="text-center" >客户联系方式</th>
...
<div class="noneshowlist text-center" style="height:auto;line-height:normal;">
 ...
 get_list_btn($menu_info['id'],$row); ?>
					<?php
 if($row['status'] == 10){ ?>
					<a href="<?=U('cancel',array('ids'=>$row['id']))?>" class="btn btn-xs btn-danger ajax-get confirm" title="确定取消">确定取消</a>
					<?php
 } if($row['status'] == 20){ ?>
					<a href="#" class="btn btn-xs bg-purple" title="已回收">已回收</a>
					<?php
 } if($row['status'] == 30){ ?>
					<a href="#" class="btn btn-xs btn-warning " title="已取消">已取消</a>
					<?php
 } ?>
				</td>
			</tr>
