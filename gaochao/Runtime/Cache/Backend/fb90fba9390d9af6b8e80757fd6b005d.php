			...
			<tr>
				<th colspan="20" class="clearfix">
					<form action="<?=U('index');?>" class="form-inline pull-left" method="get">
						<input type="text" class="form-control input-sm" placeholder="手机|昵称|姓名|车牌号" name="keyword" value="<?=$keyword?>">
						<?php
 if(!session('is_owner')){ ?>
...
<!-- 				<td class="text-center">
	<?=$type[$row['type']]['title']?>
				...
				<td>
					<?=$row['login_time'].' | ' . ip_to_location($row['login_ip']) ?>
				</td>
				<td>
					<?=$row['car_num']?>
				</td>
