...
<div class="message_bg js_message_bg"></div>
 ...
 if($pid){ $menu = get_no_del('menu'); $pid = $pid ? $pid : 1; $path = trim($menu[$pid]['path'], '-'); $structure = explode("-",$path); $structure[] = $pid; foreach($structure as $val){ if($val){ echo '<a href="'.U('index',array("pid"=>$menu[$val]['id'],"level"=>$menu[$val]['level']+1)).'">'.$menu[$val]['title'].'</a> &gt; '; } } } echo ACTION_NAME == 'add' ? '添加' : '修改'; ?>
	...
	<div class="box box-body">
		<form action="" class="js_form" method="post">
			...
			<input type='hidden' name='old_pid' value='<?=intval(I('pid'))?>' />
			...
			<label class="form-group">
				...
				<div>
					...
					<span class="Validform_checktip">选择上级菜单</span>
				</div>
			</label>
			<label class="form-group">
				<div class="control-label">
 ...
 if(!$info['id']){ ?>
			<div class="form-group form-inline">
				<span>操作子菜单</span>
				<label><input type="checkbox" name="child_menu[]" value='add'>添加</label>
				<label><input type="checkbox" name="child_menu[]" value='edit'>修改</label>
				<label><input type="checkbox" name="child_menu[]" value='enable'>启用</label>
				<label><input type="checkbox" name="child_menu[]" value='disable'>禁用</label>
				<label><input type="checkbox" name="child_menu[]" value='del'>删除</label>
				<label><input type="checkbox" name="child_menu[]" value='recommend'>推荐</label>
				<label><input type="checkbox" name="child_menu[]" value='no_recommend'>取消推荐</label>
				<label><input type="checkbox" name="child_menu[]" value='count'>统计</label>
			</div>
			<?php
 } ?>
		</div>
