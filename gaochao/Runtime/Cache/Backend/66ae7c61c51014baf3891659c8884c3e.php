			...
			<tr>
				<th colspan="8" class="clearfix">
					<form action="<?=U('index');?>" class="form-inline pull-left clearfix" method="get">
						<input type="search" class="form-control input-sm" placeholder="标题关键字" name="search" value="<?=$search?>">
...
<div class="noneshowlist text-center" style="height:auto;line-height:normal;">
		<h4><?=$tips?></h4>
			...
			<?php
 $admin = get_no_del('admin'); foreach($list as $row){ ?>
			<tr>
				...
				<td>
					<a target="_blank" href="<?=strpos($row['url'],'.')?$row['url']:U($row['url'])?>"><?=$row['url']?></a>
				</td>
