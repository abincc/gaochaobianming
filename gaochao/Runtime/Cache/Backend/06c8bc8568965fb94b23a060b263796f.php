...
<div class="message_bg js_message_bg"></div>
 ...
 if($small_tips){ ?>
	...
	<div class="box box-body">
		<form role="form" action="" method="post" class="product_form js_form">
		<input name="ids" type="hidden" value="<?=$info['id']?>" />
		<div class="form-group">
			<div class="control-label">
				商品名称：
			</div>
			<div>
				<input type="text" class="form-control width_auto" value="<?=$info['title']?>" readonly="readonly" />
			</div>
		</div>	

<?php
 $i = 1; foreach($spec_value as $v) { ?>
		<div class="form-group">
			<div class="control-label">
			...
			<div>
<?php
 $config = array( 'index' => $i++, 'table' => 'product_sku', 'name' => 'spec_value_'.$v['id'], 'val_key' => 'image', 'tpl' => 'product' ); echo W('Common/Img/index', array($config, $v['sku_id'])); ?>				
			</div>
		</div>
<?php
 } ?>
		<div class="form-group">
			...
			<div>
<?php
 $config = array( 'index' => $i, 'table' => 'product_image', 'table_id' => 'product_id', 'name' => 'image', 'val_key' => 'image', 'multi' => 'true', ); echo W('Common/Img/index', array($config, $info['id'])); ?>				
			</div>
		</div>
