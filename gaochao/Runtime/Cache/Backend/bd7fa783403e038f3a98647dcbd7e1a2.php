	...
	<table class="table js_table table-bordered">
			...
			<tr>
				<th colspan="20" class="clearfix">
					<form action="<?=U('index');?>" class="form-inline pull-left" method="get">
						<input type="text" class="form-control input-md" style="width: 250px;" placeholder="用户姓名" name="keywords" value="<?=$keywords?>">
		...
		<tbody>
			<tr role="row">
				...
				<th width="8%" class="text-center">小区</th>
				<th width="8%" class="text-center">用户名</th>
				<th width="8%" class="text-center">联系方式</th>
				<th width="10%" class="text-center">报销类型</th>
				<th width="8%" class="text-center">报销事由</th>
				<th width="6%" class="text-center">发生时间</th>
				<th width="4%" class="text-center">费用</th>
...
<div class="noneshowlist text-center" style="height:auto;line-height:normal;">
 ...
 foreach($list as $row){ ?>
			<tr>
				...
				<td class="text-center"><?=$type_s[$row['type']]['remark']?></td>
				<td class="text-center"><?=$row['reason']?></td>
				<td class="text-center"><?=substr($row['occur_time'], 0, 10)?></td>
				<td class="text-center"><?=$row['cost']?></td>
				<td class="text-center">
					<img src="/gaochao/Public/Backend/img/status_<?=$row['is_confirm']?>.png" alt=<?=$row['is_confirm']==0?"未确认":"已确认"?> title=<?=$row['is_confirm']==0?"未确认":"已确认"?>>
				</td>
				<td class="text-center">
				<?php
