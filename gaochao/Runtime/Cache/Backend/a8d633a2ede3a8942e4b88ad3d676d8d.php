			...
			<tr>
				<th colspan="9" class="clearfix">
					<form action="<?=U('index');?>" class="form-inline pull-left" method="get">
						<input type="ope_date" style="min-width:auto;width:140px!important" onclick="WdatePicker({skin:'backend',dateFmt:'yyyy-MM-dd'})" class="form-control input-sm wdate" placeholder="体检日期" name="ope_date" value="<?=I('ope_date')?>">
						<input type="text" class="form-control input-md" style="width: 250px;" placeholder="体检人姓名|联系电话" name="keywords" value="<?=$keywords?>">
						<?php
 if(!session('is_owner')){ ?>
<!-- 						<select name="district_id" class="form-control input-md super_select2" data-placeholder="请选择小区">
						...
						<input type="start_date" style="min-width:auto;width:140px!important" onclick="WdatePicker({skin:'backend',dateFmt:'yyyy-MM-dd'})" class="form-control input-sm wdate" placeholder="提交开始时间" name="start_date" value="<?=$start_date?>">
						<input type="stop_date" style="min-width:auto;width:140px!important" onclick="WdatePicker({skin:'backend',dateFmt:'yyyy-MM-dd'})" class="form-control input-sm wdate" placeholder="提交结束时间" name="stop_date" value="<?=$stop_date?>">
		...
		<tbody>
			<tr role="row">
				...
				<th width="20%">体检日期</th>
				<th width="20%">体检人姓名</th>
				<th width="20%" class="text-center">联系电话</th>
				<th width="16%" class="text-center">提交时间</th>
...
<!-- 				<td><?=$row['email']?></td> -->
				...
				<td class="text-center"><img src="/gaochao/Public/Backend/img/status_<?=$row['is_hid']?>.png" alt=""></td>
