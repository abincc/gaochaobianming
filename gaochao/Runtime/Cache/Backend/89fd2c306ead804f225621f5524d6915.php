	...
	<div class="box box-body">
		<form action="" method="post" class="js_form">
            ...
            <div class="form-group">
                ...
                <div>
                <select name="type" class="form-control input-md super_select2 width_auto" data-placeholder="请选择类型">
			...
			<label class="form-group">
				...
				<div>
					...
					<span class="Validform_checktip">标题</span>
				</div>
			</label>
			<div class="form-group">
				<div class="control-label">
				...
				<div>
					<textarea class="form-control width_auto" name="desc" datatype="*0-100" errormsg="请输入描述"><?=$info['desc']?></textarea>
					<span class="Validform_checktip">可空，100字符以内</span>
				</div>
			</div>
			<!-- <div class="form-group">
				<div class="control-label">
				...
				<div>
					<?php
 $config = array( 'index' => 1, 'table' => 'ck_home', 'table_id' => 'id', 'name' => 'cover', 'multi' => 'false', 'val_key' => 'cover' ); echo W('Common/Img/index', array($config, $info['id'])); ?>
...
if(typeof(UEDITOR_CONFIG)!="object"){
	...
	var config={
		serverUrl:"<?=U('ueditor')?>",
					...
					<!-- 百度编辑器 S -->
					<?php $time=time(); ?>
					<script id="<?='editor_'.$time?>" type="text/plain" name="remark"><?=$info['remark']?></script>
					<script type="text/javascript">
					    var config_2={
					        ...
					        pasteplain:true,
					        toolbars: [[
					               'fullscreen','bold', 'italic', 'underline', 'removeformat','indent','forecolor','justifyleft', 'justifycenter', 'justifyright', 'justifyjustify','link','insertcode', 'unlink','insertimage','attachment','preview'
					        ]]
					    };
					    //toolbars配置编辑器具体有哪些功能
              //合并配置文件，config为默认配置，config_2为自定义的配置，更多相关编辑器功能配置与含义见项目目录下的/Public/Plugins/ueditor/ueditor.config.js文件
