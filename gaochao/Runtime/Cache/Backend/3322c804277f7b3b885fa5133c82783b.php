...
<div class="message_bg js_message_bg"></div>
 ...
 if($small_tips){ ?>
	...
	<div class="structure">
		<?php
 $path=trim($parent['path'],'-'); $structure=explode("-",$path); $menu = get_no_del('menu'); echo '<a href="'.U('index').'">菜单管理</a> &gt; '; foreach($structure as $val){ if($val){ echo '<a href="'.U('index',array("pid"=>$menu[$val]['id'],"level"=>$menu[$val]['level']+1)).'">'.$menu[$val]['title'].'</a> &gt; '; } } echo $menu[$parent['id']]['title']; ?>
	</div>
 ...
 if(I('get.level', 0, 'int') > 0) { ?>
						<a href="<?=U('index', array('pid'=> $parent['pid'],'level'=>$parent['level']))?>" class="btn btn-default btn-sm">返回上一级</a>
						<?php
 } ?>
...
<div class="noneshowlist text-center" style="height:auto;line-height:normal;">
 ...
 foreach($list as $row){ ?>
			<tr>
				...
				<?php
 if(I('level')>1){ echo '<td>' . $row['url'] . '</td>'; } ?>
