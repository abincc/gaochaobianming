...
<div class="message_bg js_message_bg"></div>
 ...
 if($small_tips){ ?>
	...
	<table class="table js_table table-bordered">
		<thead>
			<tr>
				<th colspan="9" class="text-right clearfix">
					<form action="<?=U('index');?>" class="form-inline pull-left" method="get">
						<input type="text" class="form-control input-sm" placeholder="标题|用户昵称" name="keywords" value="<?=$keywords?>">
<!-- 						<select name="thread_id" class="form-control input-sm super_select2" data-placeholder="请选择分类">
		...
		<tbody>
			<tr role="row">
				...
				<th width="6%" class="text-left">发布人</th>
...
<div class="noneshowlist text-center" style="height:auto;line-height:normal;">
 ...
 foreach($list as $row){ ?>
			...
			<td><input type="checkbox" class="ids" data-child="ids" name="ids[]" value="<?=$row['id']?>"></td>
				<td><?=$row['content']?></td>
				<td class="text-left"><?=$row['member_nickname']?></td>
