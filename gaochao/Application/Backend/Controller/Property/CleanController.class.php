 ...
 * @time 2016-06-30
 */
class CleanController extends IndexController {
	...
	public function index(){
		...
		$result['service'] = get_no_hid('clean_service','sort desc,id asc');
	...
	private function _search(){
		...
		$clean_service_id  = I('clean_service_id','','int');
		$status 		   = I('status',0,'int');

		if(!empty($keyword)){
			$map['mobile|username']   = array('LIKE',"%$keyword%");
		}
		if(!empty($clean_service_id)){
			$map['clean_service_id'] = $clean_service_id;
		}
		/**小区*/
	...
	protected function operate(){
		...
		$data['service'] = get_no_hid('clean_service','sort desc,id asc');
