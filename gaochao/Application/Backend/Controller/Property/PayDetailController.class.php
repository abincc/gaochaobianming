 ...
 * @time 2016-06-30
 */
class PayDetailController extends IndexController {
    ...
    public function index(){
        ...
        $result = $this->page(D($this->table_view),$map,'district_id desc, pay_time desc');
     ...
     */
     ...
     public function completed(){
        ...
        try{
            ...
            $info = M('Incepx')->where(array('district_id'=>$order_data['district_id']))->order('add_time desc')->find();
            if(empty($info)){
                $map = array(
                ...
                $map = array(
                    'income' => $res_dat['money_total'],
                    'district_id' => $res_dat['district_id'],
                    'total' => floatval($info['total']) + floatval($res_dat['money_total']),
                    'remark'=> $res_dat['username'].'--#'.$res_dat['room_no'].'--'.$res_dat['pay_start_time'].'至'.$res_dat['pay_end_time'].'--物业费',
                    'status' => 1,
                    'add_time' => date('Y-m-d H:i:s'),
                );
