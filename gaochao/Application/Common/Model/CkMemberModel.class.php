 ...
 * 用户模型
 */
class CkMemberModel extends Model {

    protected $tableName  = 'ck_member';
    ...
    public function check_login($mobile,$password,$type){
        ...
        $has = $this->where(array('mobile'=>$mobile))->field(['id','password','is_hid','is_del'])->find();
    ...
    public function login($uid){
        ...
        $uid = intval($uid);
        /* 检测是否在当前应用注册 */
        $user = $this->field('id as member_id,username,nickname,mobile,sex,district_id')->find($uid);
        if(empty($user)){ 
    ...
    */
    public function check_token($posts){
        ...
        // }
        /* 判断登录过期(同时登录) */ 
        $login_time = M('ck_member')->where('id='.$token[0])->getField('login_time');
        if(!$login_time){
            $this->status = '-1';
            $this->error ='用户信息不存在';
            return false;
