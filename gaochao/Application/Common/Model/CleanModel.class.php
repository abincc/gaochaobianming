...
class CleanModel extends Model {
    ...
    protected $_validate = array(
        ...
        array('clean_service_id', 'require', '请选择服务类型', self::EXISTS_VALIDATE, 'regex', self::MODEL_BOTH),
    ...
    */
    public function get_covers($ids){
        ...
        $res = M('clean_image')->where(array('pid'=>array('IN',$ids)))->field(['pid','image'])->group('pid')->select();
        if(!empty($res)){
    ...
    public function get_image($pid){
        return (array)M('clean_image')->where('pid='.intval($pid))->field(['id as image_id','image'])->order('add_time desc')->select();
    }
    ...
    */
    ...
    public function get_reply_image($pid){
        return (array)M('clean_reply_image')->where('pid='.intval($pid))->field(['id as image_id','image','width','height'])->order('add_time desc')->select();
    }
}
